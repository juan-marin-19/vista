...
- **`ModuleNotFoundError: No module named 'config'`**: asegúrate de ejecutar `python manage.py` desde `backend/`. Esta versión usa `vrisa.config` como módulo de configuración.
- **Docker Desktop en Windows**: el error `open //./pipe/dockerDesktopLinuxEngine` indica que Docker Desktop no está iniciado o no tiene el motor Linux habilitado. Abre Docker Desktop y espera a que diga “Docker is running”.
